...
fn linker_with_args<'a>(
    ...
    let mut cmd = get_linker(path.map(|path| path.into_std_path_buf()), target);
...
fn get_linker<'a>(
    ...
    Box::new(UnifiedLinker { cmd, target }) as Box<dyn Linker>
}
...
pub struct UnifiedLinker<'a> {
    cmd: Command,
    target: &'a Target,
}

impl<'a> UnifiedLinker<'a> {
...
impl<'a> Linker for UnifiedLinker<'a> {
    fn cmd(&mut self) -> &mut Command {
        &mut self.cmd
    }
